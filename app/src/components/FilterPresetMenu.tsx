    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          ...
          className="absolute left-0 top-[calc(100%+6px)] z-20 w-72 overflow-hidden rounded-xl border border-border bg-surface shadow-lg"
        >
          {presets.length > 0 && (
                  ...
                  <li key={p.id} className="group">
                    <div
                      className={`flex items-center gap-2 px-3 py-1.5 text-xs ${
                        isActive ? "bg-brand-soft" : "hover:bg-bg-muted"
                      }`}
                    >
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <span className={`truncate ${isActive ? "text-brand" : "text-fg"}`}>
                          {p.name}
                        </span>
                      </button>
                      <button
                        type="button"
                        onClick={() => onDelete(p)}
                        className="rounded p-1 text-fg-subtle opacity-0 transition-opacity hover:text-danger group-hover:opacity-100 cursor-pointer"
          ...
          {canSave && (
            <div
              className={`border-t border-border px-3 py-2 ${
                presets.length === 0 ? "" : "bg-bg-muted/30"
              }`}
            >
              <p className="mb-1.5 text-[10px] font-medium uppercase tracking-[0.16em] text-fg-faint">
              ...
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  className="flex-1 rounded-md border border-border-strong bg-surface-active px-2 py-1 text-xs text-fg placeholder:text-fg-subtle"
                  autoFocus
                />
                <button
                  type="button"
                  ...
                  className="inline-flex items-center gap-1 rounded-md bg-brand px-2 py-1 text-xs font-medium text-brand-fg disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer"
                >
              ...
              {saveError && (
                <p className="mt-1 text-[11px] text-danger">{saveError}</p>
              )}
            </div>
          )}
